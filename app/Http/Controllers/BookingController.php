            ...
            foreach ($data['seats'] as $seatCode) {
                ...
                foreach ($codesToSave as $code) {
                    ...
                    if ($seat) {
                        ...
                        try {
                            if (Schema::hasTable('tam_giu_ghe')) { // Lưu ý: Tên bảng trong SQL dump của bạn là 'tam_giu_ghe'
                                $showtimeSeat = ShowtimeSeat::where('id_suat_chieu', $data['showtime'])
                                    ->where('id_ghe', $seat->id)
                                    ->first();

                                // Xác định trạng thái ghế dựa trên phương thức thanh toán
                                // Nếu thanh toán tại quầy -> Booked (để giữ chỗ cứng cho đến khi nhân viên check)
                                // Nếu Online -> Booked (nhưng vé pending, nếu hủy thì vnpayReturn sẽ xóa)
                                $seatStatus = 'booked';
                                $seatExpire = null; // Không bao giờ hết hạn tự động, chờ nhân viên hủy hoặc xác nhận

                                if ($showtimeSeat) {
                                    $showtimeSeat->trang_thai = $seatStatus;
                                    $showtimeSeat->thoi_gian_het_han = $seatExpire;
                                    $showtimeSeat->save();
                                } else {
                                    ShowtimeSeat::create([
                                        'id_suat_chieu' => $data['showtime'],
                                        'id_ghe' => $seat->id,
                                        'trang_thai' => $seatStatus, // Cột này là 'trang_thai' hay 'status' tùy model của bạn
                                        'thoi_gian_het_han' => $seatExpire,
                                        'thoi_gian_giu' => now(),
                                    ]);
                                }
                            }
                        } catch (\Throwable $e) {
                            \Log::warning('Lỗi cập nhật tam_giu_ghe: ' . $e->getMessage());
                        }
