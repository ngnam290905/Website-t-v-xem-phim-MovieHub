...
@section('page-description', 'Danh sách các suất chiếu (Chế độ xem)')
  ...
  <div class="space-y-6">
      ...
      <div class="flex items-start gap-3">
        ...
        <div>
          ...
          <p class="text-blue-200 text-sm mt-1">
            Bạn đang xem danh sách suất chiếu. Chỉ có thể xem thông tin, không thể chỉnh sửa.
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="bg-[#151822] border border-[#262833] rounded-xl p-4">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <input type="text" placeholder="Tìm theo tên phim..." class="w-full px-3 py-2 bg-[#1a1d24] border border-[#262833] rounded-lg text-white placeholder-[#a6a6b0] focus:outline-none focus:border-[#F53003]">
        </div>
        <div>
        ...
        <div>
          <label class="block text-sm font-medium text-[#a6a6b0] mb-2">Trạng thái</label>
          <select class="w-full px-3 py-2 bg-[#1a1d24] border border-[#262833] rounded-lg text-white focus:outline-none focus:border-[#F53003]">
          ...
          <tbody class="divide-y divide-[#262833]">
            ...
            <tr class="hover:bg-[#1a1d24] transition-colors">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <div class="flex-shrink-0 h-12 w-8 bg-[#262833] rounded flex items-center justify-center">
                    <i class="fas fa-film text-[#a6a6b0]"></i>
                  </div>
                  <div class="ml-4">
                    <div class="text-sm font-medium text-white">{{ $suat->phim->ten_phim ?? 'N/A' }}</div>
                    <div class="text-sm text-[#a6a6b0]">{{ $suat->phim->dao_dien ?? 'N/A' }}</div>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-white">{{ $suat->phongChieu->ten_phong ?? 'N/A' }}</div>
                <div class="text-sm text-[#a6a6b0]">{{ $suat->phongChieu->suc_chua ?? 'N/A' }} ghế</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-white">{{ \Carbon\Carbon::parse($suat->thoi_gian_bat_dau)->format('d/m/Y H:i') }}</div>
                <div class="text-sm text-[#a6a6b0]">{{ \Carbon\Carbon::parse($suat->thoi_gian_ket_thuc)->format('H:i') }}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                @if($suat->trang_thai)
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                <a href="{{ route('staff.suat-chieu.show', $suat->id) }}" class="text-blue-400 hover:text-blue-300 mr-3">
            ...
            <tr>
              <td colspan="5" class="px-6 py-12 text-center">
                <div class="text-[#a6a6b0]">
                  ...
                  <p class="text-lg">Không có suất chiếu nào</p>
                  <p class="text-sm">Chưa có dữ liệu suất chiếu trong hệ thống</p>
                </div>
              </td>
            </tr>
            @endforelse
    ...
    <div class="flex items-center justify-between">
      <div class="text-sm text-[#a6a6b0]">
        Hiển thị {{ $suatChieu->firstItem() }} đến {{ $suatChieu->lastItem() }} trong {{ $suatChieu->total() }} kết quả
      </div>
