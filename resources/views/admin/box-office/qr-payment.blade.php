...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('admin.box-office.index') }}" class="text-gray-400 hover:text-white">
    ...
    <div class="bg-[#151822] border border-[#262833] rounded-2xl p-8">
        ...
        <div class="flex justify-center gap-4">
            <button id="cancel-btn" 
                    onclick="window.location.href='{{ route('admin.box-office.index') }}'"
...
document.getElementById('confirm-payment-btn').addEventListener('click', function() {
    ...
    fetch('{{ route("admin.box-office.payments.confirm") }}', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            ...
            alert('Thanh toán thành công! Đang chuyển đến trang in vé...');
            
            // Redirect đến trang in vé
            if (data.redirect_url) {
                window.location.href = data.redirect_url;
            } else {
                window.location.href = '{{ route("admin.box-office.tickets.print", $booking->id) }}';
            }
        } else {
