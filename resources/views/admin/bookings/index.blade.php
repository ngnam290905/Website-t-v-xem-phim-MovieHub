            ...
            <form method="GET" action="{{ route('admin.bookings.index') }}" class="w-full">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-3">
                    ...
                    <div class="lg:col-span-1 flex items-end gap-2">
                        ...
                        @if (request()->hasAny(['status', 'phim', 'nguoi_dung', 'booking_date', 'show_date', 'show_time']))
                            <a href="{{ route('admin.bookings.index') }}"
                                class="bg-gray-700 hover:bg-gray-600 text-gray-300 rounded-lg text-sm px-3 py-2.5 transition duration-200"
                                title="Xóa lọc">
