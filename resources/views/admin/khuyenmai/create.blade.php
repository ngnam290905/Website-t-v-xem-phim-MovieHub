...
<div class="max-w-xl mx-auto bg-[#151822] p-6 rounded-xl border border-[#262833]">
    <h1 class="text-2xl font-bold mb-6">Thêm mã khuyến mãi</h1>
    <form action="{{ route('admin.khuyenmai.store') }}" method="POST">
        @csrf
        <div class="mb-4">
            <label class="block mb-1 font-semibold">Mã khuyến mãi</label>
            <input type="text" name="ma_km" class="w-full px-3 py-2 rounded bg-[#222533] border border-[#262833] focus:outline-none" value="{{ old('ma_km') }}">
            @error('ma_km')<div class="text-red-500 text-sm mt-1">{{ $message }}</div>@enderror
        </div>
        <div class="mb-4">
            <label class="block mb-1 font-semibold">Mô tả</label>
            <textarea name="mo_ta" class="w-full px-3 py-2 rounded bg-[#222533] border border-[#262833] focus:outline-none">{{ old('mo_ta') }}</textarea>
        ...
        <div class="mb-4 flex gap-4">
            <div class="flex-1">
                ...
                <input type="date" name="ngay_bat_dau" class="w-full px-3 py-2 rounded bg-[#222533] border border-[#262833] focus:outline-none" value="{{ old('ngay_bat_dau') }}">
            ...
            <div class="flex-1">
                ...
                <input type="date" name="ngay_ket_thuc" class="w-full px-3 py-2 rounded bg-[#222533] border border-[#262833] focus:outline-none" value="{{ old('ngay_ket_thuc') }}">
        ...
        <div class="mb-4 flex gap-4">
            ...
            <div class="flex-1">
                <label class="block mb-1 font-semibold" id="label_gia_tri_giam">
                    {{ old('loai_giam', 'phantram') == 'codinh' ? 'Giá trị giảm (VNĐ)' : 'Giá trị giảm (%)' }}
                </label>
                <input type="number" name="gia_tri_giam" id="gia_tri_giam" class="w-full px-3 py-2 rounded bg-[#222533] border border-[#262833] focus:outline-none" value="{{ old('gia_tri_giam') }}">
            ...
            <div class="flex-1">
                ...
                <input type="text" name="dieu_kien" class="w-full px-3 py-2 rounded bg-[#222533] border border-[#262833] focus:outline-none" value="{{ old('dieu_kien') }}">
        ...
        <div class="flex justify-end gap-2">
            ...
            <button type="submit" class="px-4 py-2 rounded bg-green-600 hover:bg-green-700 text-white font-semibold">Lưu</button>
        </div>
    </form>
</div>
@endsection
