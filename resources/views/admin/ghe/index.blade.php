...
@section('content')
  ...
  <div class="space-y-6">
    ...
    <div class="bg-[#151822] border border-[#262833] rounded-xl p-4">
      <form method="GET" action="{{ route('admin.ghe.index') }}" class="flex flex-wrap items-end gap-4">
        <div>
          <label class="block text-sm font-medium text-white mb-2">Lọc theo phòng</label>
          <select name="id_phong" class="w-64 px-3 py-2 bg-[#1a1d24] border border-[#262833] rounded-md text-white">
            <option value="">Tất cả phòng</option>
            @foreach(($rooms ?? []) as $room)
              <option value="{{ $room->id }}" {{ request('id_phong') == $room->id ? 'selected' : '' }}>{{ $room->ten_phong }}</option>
            @endforeach
          </select>
        </div>
        <div class="flex items-center gap-2">
          <button type="submit" class="px-4 py-2 bg-[#F53003] hover:bg-[#e02a00] text-white rounded-md">Tìm kiếm</button>
          <a href="{{ route('admin.ghe.index') }}" class="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-md">Xóa bộ lọc</a>
        </div>
      </form>
    </div>
    ...
    <div class="bg-[#151822] border border-[#262833] rounded-xl overflow-hidden">
      <div class="overflow-x-auto">
        <form method="POST" action="{{ route('admin.ghe.bulk') }}" id="bulkForm">
        @csrf
        <div class="p-4 bg-[#1a1d24] border-b border-[#262833] flex flex-wrap items-center gap-3">
          <span class="text-white text-sm">Thao tác hàng loạt:</span>
          <select name="action" class="px-3 py-2 bg-[#151822] border border-[#262833] rounded-md text-white" id="bulkAction">
            <option value="">Chọn hành động</option>
            <option value="lock">Khóa ghế</option>
            <option value="unlock">Mở ghế</option>
            <option value="type">Đổi loại ghế</option>
            <option value="delete">Xóa ghế</option>
          </select>
          <select name="id_loai" class="px-3 py-2 bg-[#151822] border border-[#262833] rounded-md text-white" id="bulkSeatType" style="display:none;">
            <option value="">Chọn loại ghế</option>
            @foreach(($seatTypes ?? []) as $loai)
              <option value="{{ $loai->id }}">{{ $loai->ten_loai }} - {{ $loai->he_so_gia }}x</option>
            @endforeach
          </select>
          <button type="submit" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-md" onclick="return confirm('Xác nhận thực hiện hành động cho các ghế đã chọn?')">Thực hiện</button>
        </div>
        <table class="min-w-full divide-y divide-[#262833]">
          ...
          <tbody class="bg-[#151822] divide-y divide-[#262833]">
            ...
            <tr class="hover:bg-[#1a1d24] transition-colors duration-200">
              <td class="px-6 py-4"><input type="checkbox" name="seat_ids[]" value="{{ $seat->id }}" class="rowCheck w-4 h-4"></td>
