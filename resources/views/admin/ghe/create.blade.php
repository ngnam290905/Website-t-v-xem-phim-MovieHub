...
@section('title', 'Tạo Ghế Mới')

@section('content')
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <h1 class="text-2xl font-bold">Tạo Ghế Mới</h1>
    ...
    <div class="bg-[#151822] border border-[#262833] rounded-xl p-6">
      <form action="{{ route('admin.ghe.store') }}" method="POST" class="space-y-6">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <select name="id_loai" id="id_loai" class="w-full px-3 py-2 bg-[#1a1d24] border border-[#262833] rounded-md text-white focus:outline-none focus:ring-2 focus:ring-[#F53003] @error('id_loai') border-red-500 @enderror" required>
              ...
              <option value="{{ $loai->id }}" {{ old('id_loai') == $loai->id ? 'selected' : '' }}>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            ...
            <input type="number" name="so_hang" id="so_hang" 
                   ...
                   value="{{ old('so_hang') }}" min="1" required>
            @error('so_hang')
              <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            @enderror
          </div>
          ...
          <div>
            ...
            <input type="number" name="so_cot" id="so_cot" 
                   ...
                   value="{{ old('so_cot') }}" min="1" required>
            @error('so_cot')
              <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            @enderror
          </div>
        ...
        <div class="flex justify-end space-x-3">
          ...
          <button type="submit" class="px-4 py-2 bg-[#F53003] hover:bg-[#e02a00] text-white rounded-md transition-colors duration-300">
            <i class="fas fa-save mr-2"></i>Tạo Ghế
          </button>
        </div>
      </form>
    </div>
